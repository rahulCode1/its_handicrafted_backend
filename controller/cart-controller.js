...
const decreaseQuantity = async (req, res, next) => {
    ...
    if (!error.isEmpty()) {
        return next(new HttpError(
            error.array()[0].msg ||
            "Product id required to decrease quantity.",
            422
        ))
    }
    const userId = req.params.userId
    ...
    try {
        ...
        const existingItem = cart.items.find(product => product.product.toString() === productId)
        if (!existingItem) {
            return next(new HttpError("Product not found in cart", 404))
        }
        if (existingItem.quantity === 1) {

            cart.items = cart.items.filter(product => product.product.toString() != productId)
        } else {
            existingItem.quantity -= 1
        }
        await cart.save()
...
const removeFromCart = async (req, res, next) => {
    ...
    if (!error.isEmpty()) {
        return next(new HttpError(
            error.array()[0].msg ||
            "Product id required to remove from cart.",
            422
        ))
    }
...
const moveToWishlist = async (req, res, next) => {
    ...
    if (!error.isEmpty()) {
        return next(new HttpError(
            error.array()[0].msg ||
            "Product id required to move to wishlist.",
            422
        ))
    }
    ...
    try {
        ...
        const existingCartItem = cart.items.find(product =>
